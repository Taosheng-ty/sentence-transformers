...
class sbertSave(SentenceTransformer):
    def __init__(self,patience=20,*args, **kwargs):
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                ...
                if save_best_model:
                    self.save(os.path.join(output_path,"epoch-"+str(epoch)+"steps-"+str(steps))+"-Score"+str(score)) 
            else:
                self.evalEpoch+=1
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            if self.evalEpoch>self.patience:
                logger.info("exceeds the patience for validation of {valiEpoch}".format(valiEpoch=self.patience))
                break 
